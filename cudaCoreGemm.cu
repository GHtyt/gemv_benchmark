...
#pragma unroll
    ...
    int woffset = 0;
    int aoffset = 0;
    for (int idxK = 0; idxK < KM; ++idxK)
    {
        for (int i = 0; i < TILE_N; ++i)
        {
            //auto tile_w_quantized = reinterpret_cast<VecType const*>(weight + i * kTileK + idxK * TILE_N + tid * kStepK)[0];
            auto tile_w_quantized = reinterpret_cast<VecType const*>(weight + woffset + idk)[0];
#pragma unroll
...
#pragma unroll
        ...
        {
            //auto tile_a_quantized = reinterpret_cast<VecType const*>(act + i * kTileK + idxK * TILE_M + tid * kStepK)[0];
            auto tile_a_quantized = reinterpret_cast<VecType const*>(act + aoffset + idk)[0];
#pragma unroll
            for (int cvtIdx = 0; cvtIdx < kCvtCount; ++cvtIdx)
            {
