...
int main(int argc, char* argv[]) {
    ...
    printf("w4a16g bs : %d    IC = %d   OC = %d    ", m, k, n);

    int groupsize = 128;
    auto KV = KernelType::FP16Int4Groupwise;
    //int groupsize = 0;
    //auto KV = KernelType::FP16Int4PerChannel;
    using AType = half;
    using WType = cutlass::uint4b_t;
    ...
    WType*   h_weight    = (WType  *)malloc(k * n * sizeof(WType));
    ...
    cudaMalloc(&d_weight,     k * n             * sizeof(WType));
    ...
    cudaMemcpy(d_weight,     h_weight,     k * n             * sizeof(WType),    cudaMemcpyHostToDevice);
