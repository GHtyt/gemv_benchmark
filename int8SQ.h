...
struct Params
{
    ...
    Params(int8_t const* _act, int8_t const* _weight, float const* _scale_tokens, float const* _scale_channels,
        void* _output, int* _idx, int* _scale, int _m, int _n, int _k, bool _per_channel, bool _per_token)
        : act(_act)
        , weight(_weight)
...
__global__ void int8_sq(int8_t const* act, int8_t const* weight, float const* scale_channels, float const* scale_tokens,
    Type* output, int* idx, int* scale, int m, int n, int k)
{
    using VecType = int4;
    static constexpr int kStepK = sizeof(VecType) / sizeof(int8_t);//128 / (8 * sizeof(int8_t));
...
void int8_sq_kernel(Params& params, cudaStream_t s)
{
    ...
    int8_sq<Type, CtaM, CtaN, Threads, PerChannel, PerToken><<<grid, block, 0, s>>>(params.act, params.weight,
        params.scale_channels, params.scale_tokens, reinterpret_cast<Type*>(params.output), params.idx, params.scale, 
...
#define DISPATCH(PerChannel, PerToken)                                                                                 \
    ...
    bool per_channel = params.per_channel;
    bool per_token = params.per_token;
